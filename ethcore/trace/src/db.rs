...
mod tests {
	...
	fn test_import() {
		...
		assert_eq!(tracedb.trace(1, 0, Vec::new()).unwrap(), create_simple_localized_trace(1, block_1, tx_1));
		assert_eq!(tracedb.trace(2, 0, Vec::new()).unwrap(), create_simple_localized_trace(2, block_2, tx_2));
	}
	...
	fn query_trace_after_reopen() {
		...
		extras.block_hashes.insert(0, H256::zero());
		extras.transaction_hashes.insert(0, Vec::new());
